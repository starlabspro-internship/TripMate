...
class MessagesController extends Controller
{
    ...
    public function send(Request $request)
    {
        ...
        // If no matching booking-trip combination is found, deny access
        if (!$canMessageDriver) {
            return response()->json([
                'status' => '403',
                'error' => (object)[
    ...
    public function search(Request $request)
    {
        ...
        if (Trip::where('driver_id', $userId)->exists()) {
            $driverTripIds = Trip::where('driver_id', $userId)->pluck('id');
            if ($driverTripIds->isEmpty()) {
                return Response::json([
                    'records' => '<p class="message-hint center-el"><span>You aren`t the driver for any trips.</span></p>',
                    'total' => 0,
                    'last_page' => 1
                ], 200);
            }
            $records = User::whereHas('bookings', function ($query) use ($driverTripIds) {
                $query->whereIn('trip_id', $driverTripIds)
        ...
        if (Booking::where('passenger_id', $userId)->exists()) {
            $passengerTripIds = Booking::where('passenger_id', $userId)->pluck('trip_id');
            Log::info('Passenger trip IDs: ', $passengerTripIds->toArray());
            if ($passengerTripIds->isEmpty()) {
                return Response::json([
                    'records' => '<p class="message-hint center-el"><span>You aren’t booked in any trips.</span></p>',
                    'total' => 0,
                    'last_page' => 1
                ], 200);
            }

            // Search for the driver in the trips the passenger is booked in
            $records = User::whereHas('trips', function ($query) use ($passengerTripIds) {
                $query->whereIn('id', $passengerTripIds)
                    ->where('driver_id', '!=', Auth::user()->id);  // Exclude the passenger from the results
            })
                ->where('id', '!=', $userId)
                ->where('name', 'LIKE', "%{$input}%")
                ->paginate($request->per_page ?? $this->perPage);

            // Render search results for passengers
            foreach ($records->items() as $record) {
                $getRecords .= view('Chatify::layouts.listItem', [
                    'get' => 'search_item',
                    'user' => Chatify::getUserWithAvatar($record),
        ...
        // Default case if the user is neither a driver nor a passenger (optional, you can customize this if needed)
        return Response::json([
            'records' => '<p class="message-hint center-el"><span>Invalid user role.</span></p>',
            'total' => 0,
            'last_page' => 1
        ], 200);
