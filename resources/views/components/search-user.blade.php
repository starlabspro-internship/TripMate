<div class="search-user-container relative mx-auto max-w-md md:max-w-lg lg:max-w-xl">  
    ...
    <div id="searchUserResults" class="search-results px-2 py-1 absolute w-full mt-1 bg-white border border-white rounded-md text-sm "></div>
</div>



<div id="userDetails" class="user-modal fixed inset-0 bg-black bg-opacity-50 hidden flex justify-center items-center z-50 p-4 ">
    <div class="modal-content bg-white p-4 sm:p-6 rounded-lg w-full max-w-sm md:max-w-md lg:max-w-lg">
        <span class="close text-xl cursor-pointer text-gray-500 hover:text-gray-700" onclick="closeModal()">&times;</span>
        ...
        <h2 id="modalUserName" class="text-lg md:text-2xl font-semibold flex items-center justify-center mb-6"></h2>
  ...
  (function() {
    ...
    if (users.length > 0) {
        users.forEach(user => {
            ...
            userDiv.classList.add("user-result", "p-1.5", "cursor-pointer", "hover:bg-customgreen-100");
...
document.getElementById("modalUserName").textContent = `${user.name} ${user.lastname ? user.lastname : ''}`;
...
if (user.image) {
    ...
    const lastname = user.lastname ? user.lastname : '';
    const avatarUrl = `https://eu.ui-avatars.com/api/?name=${encodeURIComponent(user.name + ' ' + lastname)}&size=128`;
    userPhoto.src = avatarUrl;
    userPhoto.alt = `${user.name} ${lastname}`;
}
        ...
        document.getElementById("modalUserName").innerHTML += ` ${verificationIcon}`;
        document.getElementById("modalUserName").innerHTML += ` ${verificationBackgroundIcon}`;
    ...
    document.querySelector("#userDetails .close").addEventListener("click", function() {
