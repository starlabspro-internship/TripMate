<x-app-layout>
    <div class="bg-white mt-5  mx-[120px] rounded-3xl">
        <form action="{{ route('bookings.store') }}" method="POST"  class="bg-white mt-10 m-5 rounded-3xl shadow-2xl">
            @csrf
            <input type="hidden" name="trip_id" value="{{ $trip->id }}">
            <input type="hidden" name="passenger_id" value="{{ auth()->user()->id }}">
                <div class="relative">
                    <div id="map" class=" py-5 h-[546px] w-1/2 md:float-end lg:block rounded-r-3xl"></div>
                </div>
                <div class="p-10">
                    <div class="flex my-2 text-black text-xl capitalize space-x-6 justify-between">
                        ...
                        <div class="flex-col  pb-6">
                            ...
                            <input class="  border border-gray-700 rounded-lg bg-gray-200 shadow-sm focus:outline-none focus:ring-2 focus:border-transparent text-gray-700"
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            .bindPopup(`
                    <div class="text-black font-medium p-3 rounded-lg text-center">
