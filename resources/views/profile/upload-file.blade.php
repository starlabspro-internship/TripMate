<x-app-layout>
    <div class="justify-center flex mt-20 mb-10">
        <div class="bg-white shadow-md rounded-lg p-6 w-4/5 sm:w-1/2">
            <h2 class="text-md sm:text-lg font-semibold text-customgreen-500 mb-4 text-center">{{ __('messages.Upload and Preview Document') }}</h2>
            <div id="drop-area"
                 class="border-dashed border-2 border-customgreen-500 bg-[#d4e9ee] rounded-lg py-12 text-center hover:bg-gray-200 transition">
                <p class="text-gray-600 font-medium">{{ __('messages.Upload your file here') }}</p>
                ...
                <form method="POST" action="{{ route('profile.background') }}" enctype="multipart/form-data" class=" p-1  mx-auto border-4 border-transparent rounded-3xl mt-4 ">
                    @csrf
                <input
                    type="file"
                    id="document-upload"
                ...
                />
            <div id="preview-container" class="mt-1 sm:mt-4  hidden">
                <h3 class="text-md font-medium text-gray-700 mb-2">{{ __('messages.Preview:') }}</h3>
                <div id="preview" class="border border-gray-300 rounded-lg overflow-hidden w-full"></div>
            </div>
            <div class="flex justify-center mt-6 gap-x-4">
                <button type="submit" class="px-4 sm:px-8 flex items-center justify-center mb-6 border bg-blue-500 hover:border-blue-900 hover:bg-blue-800 text-white text-base py-2 rounded-full tracking-wide font-medium cursor-pointer transition ease-in duration-500">
                    {{ __('messages.Upload') }}
                </button>
                ...
                <button
                    id="change-file-btn"
                    class="hidden lg:w-auto flex items-center justify-center text-base text-customgreen-300 font-medium underline hover:text-customgreen-500 transition duration-300 ease-in-out "
                    onclick="changeFile()">
        ...
        function previewFile() {
            ...
            if (file) {
                ...
                if (file.type === 'image/jpeg' || file.type === 'image/png') {
                    ...
                    reader.onload = () => {
                        previewContainer.classList.remove('hidden');
                        preview.innerHTML = `<embed src="${reader.result}" type="application/pdf" class="w-full max-h-[85vh] sm:h-[800px] h-auto object-contain overflow-y-auto items-center text-center" />`;
                    };
                    reader.readAsDataURL(file);
                }
                else {
