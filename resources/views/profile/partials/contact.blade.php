...
<form id="myForm" action="{{ route('contact.submit') }}" method="POST">
    ...
    <div class="mb-5 flex flex-col sm:flex-row space-y-4 sm:space-y-0 sm:space-x-2">
        <div class="flex-1">
            <input type="hidden" id="name" name="name" value="{{auth()->user()->name}}" class="w-full h-10 bg-gray-200 border border-gray-300 rounded-lg shadow-sm p-2 mt-1 text-sm placeholder-gray-700" >
        </div>

        <div class="flex-1">
            <input type="hidden" id="email" name="email" value="{{auth()->user()->email}}" class="w-full h-10 bg-gray-200 border border-gray-300 rounded-lg shadow-sm p-2 mt-1 text-sm placeholder-gray-700" required>
        </div>
    </div>
    ...
    <div class="mb-5">
        <select id="subject" name="subject" class="w-full h-10 bg-gray-200 border border-gray-300 rounded-lg shadow-sm p-2 mt-1 text-sm placeholder-gray-700" required>
            <option value="" disabled selected> {{ __('messages.select subject') }}</option>
            <option value="Info Request">{{ __('messages.Info Request') }}</option>
            <option value="Customer Support">{{ __('messages.Customer Support') }}</option>
            <option value="Feedback">{{ __('messages.Feedback') }}</option>
            <option value="Technical Issue">{{ __('messages.Technical Issue') }}</option>
    ...
    <!-- Message Input -->
    <div class="mb-5">
        <textarea id="message" name="message" rows="4" placeholder="{{ __('messages.message') }}" class="w-full h-32 bg-gray-200 border border-gray-300 rounded-lg shadow-sm p-2 mt-1 text-sm placeholder-gray-700" required></textarea>
        @error('message')
        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
        @enderror
    </div>
    ...
    <div class="flex justify-start mb-5">
        <button id="submitButton" type="submit" class="bg-black text-white font-semibold text-sm py-2 px-4 rounded-lg border border-transparent hover:bg-white hover:text-black hover:border-black transition duration-300 inline-flex items-center">
            <span class="button-text">{{ __('messages.Submit') }}</span>
